...
public class Bishop extends Piece {
	...
	// constructor
	public Bishop(boolean color, int row, int column) { super(color, row, column); rep = getColor()? (char) 9815: (char) 9821; }

	// methods
	@Override
	public boolean canMove() {								// if any of the following squares can be moved to, return true
		for (int i = -1; i < 2; i += 2) {
			if (canMoveToSquare(Board.getSquare(getRow() + i, getColumn() + i)) ||
	...
	public String getName() {
		return (getColor()? "White ": "Black ") + "Bishop";
		}
